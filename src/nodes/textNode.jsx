...
export const TextNode = ({ id, data, selected }) => {
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(newVariables) !== JSON.stringify(variables)) {
      console.log('Variables changed from', variables, 'to', newVariables);
  ...
  const adjustHeight = useCallback(() => {
    if (textareaRef.current) {
      ...
      console.log(`Textarea height adjusted to: ${newHeight}px`);
    }
  }, []);
  ...
  const calculateNodeHeight = () => {
    ...
    console.log(`Node height calculation: variables=${variables.length}, calculated=${calculatedHeight}`);
  ...
  return (
    <div className={`turbo-node ${selected ? 'selected' : ''}`} data-node-type="text">
      {/* Delete Button */}
      <button
      ...
      <div className="wrapper gradient" style={{ minHeight: `${nodeHeight}px` }}>
        <div className="inner">
          ...
          {variables.map((variable, index) => {
            ...
            console.log(`Creating handle ${index + 1}/${variables.length}: ${variable} at position ${topPosition}px`);
              ...
              {variables.length > 0 && (
                <div className="variables-indicator">
                  <span className="text-xs text-blue-400">
                    Variables: {variables.length} ({variables.slice(0, 3).join(', ')}{variables.length > 3 ? '...' : ''})
                  </span>
                </div>
              )}
            </div>
